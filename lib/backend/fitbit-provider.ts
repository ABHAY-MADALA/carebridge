...
export function normalizeGooglePoints(type: Point["type"], payload: unknown, importedAt: string): Point[] {
  ...
  for (const raw of rows) {
    ...
    const day = type === "sleepMinutes" ? date(record(interval.civilEndTime).date)
    ...
    const results = await Promise.all((["steps", "sleepMinutes", "restingHeartRate"] as const).map(async type => {
      try {
        ...
        for (let page = 0; page < 20; page++) {
          ...
          const body = { range: { start: civil(start), end: civil(end) }, windowSizeDays: 1, pageSize: 100, dataSourceFamily: family, ...(pageToken ? { pageToken } : {}) };
          if (type !== "steps") {
