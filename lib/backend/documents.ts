...
export class DocumentStore {
  ...
  list() {
    return this.db.sql.prepare("SELECT id,name,type,added_at,bytes FROM documents WHERE user_id=? ORDER BY added_at DESC,id").all(this.userId).map(row => {
      const id = String(row.id);
      const bytes = openBytes(Buffer.from(row.bytes as Uint8Array), `document:${this.userId}:${id}`);
      return {
        userId: this.userId, id, name: String(row.name), type: String(row.type), addedAt: String(row.added_at), size: bytes.length,
      };
    });
  }
  read(id: string) {
    ...
    const bytes = openBytes(Buffer.from(row.bytes as Uint8Array), `document:${this.userId}:${id}`);
    return { userId: this.userId, data: bytes.toString("base64") };
  }
  save(input: unknown) {
    ...
    const decoded = files.map(file => {
      ...
      try {
        inspectDocument(extension, bytes, TYPES[extension]);
      } catch {
        throw new BackendError(400, "unsafe-document-rejected");
      }
      total += bytes.length;
    ...
    for (const file of decoded) {
      ...
      if (existing) {
        const existingBytes = openBytes(Buffer.from(existing.bytes as Uint8Array), `document:${this.userId}:${file.id}`);
        if (existing.name !== file.name || !existingBytes.equals(file.bytes)) throw new BackendError(409, "document-id-conflict");
        continue;
      }
      const storedBytes = sealBytes(file.bytes, `document:${this.userId}:${file.id}`);
      this.db.sql.prepare("INSERT INTO documents VALUES (?,?,?,?,?,?)").run(this.userId, file.id, file.name, file.type, addedAt, storedBytes);
    }
    return this.list();
  }
}
